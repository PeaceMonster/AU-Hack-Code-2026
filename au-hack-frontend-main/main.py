...
import os
SERIAL_PORT = os.environ.get("SERIAL_PORT", "/dev/cu.usbmodem11101")
...
def send():
    ...
    try:
        ...
        port = serial.Serial(SERIAL_PORT, baudrate=9600, timeout=0.1)  # match your baud rate
        for v in data:
            print(v)
            port.write(f"{v}\n".encode())
            time.sleep(0.7)
...
def normalize(data: list[float]) -> list[int]:
    ...
    return list(int(((x - minv) / (maxv - minv)) * 1024) for x in data)
...
def send_serial(data: list[int]):
    ...
    for i, d in enumerate(data):
        data[i] = d * prefix[i]
    port = serial.Serial(SERIAL_PORT, baudrate=9600, timeout=0.1)
